...
GitHub allows you to manage Git repositories as a cloud based hosting service, while Git is a version control system that keeps track of changes made or an overall history of your work.
2. Why do we create a branch?
Branches are important in order to create another development line that differs from the master or main branch and allows others quickly access the specific additions that have been added to the software.
3. What is the purpose of a pull request? 
A pull request allows us to link with other users on a specific project regarding any changes or additions we would like to make to the software for review before actually integrating the changes.
4. What is the command you can use to switch between branches? For example you are working on a feature branch and you want to switch back to main. 
$ git checkout <name of branch>
5. Explain the difference between `git fetch`, `git merge` and `git pull` what does each command do? 
git fetch is used to download contents from a remote repository, git merge combines commits into one sole history of commits, and git pull updates the local version of a repository from a remote.
6. What is a merge conflict? How do you resolve a merge conflict? 
Merge conflicts may happen when multiple changes are made to the same line of a file, or when someone edits the file and another individual deletes that file. In order to resolve this you must create a new commit and choose which changes would be incorporated from the different branches.